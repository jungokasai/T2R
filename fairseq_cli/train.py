...
def main(args):
    ...
    if args.attn_only_training:
        print('Only training the following parameters')
        for name, p in model.named_parameters():
            if ("fc3" not in name) and ("fc4" not in name) and ("q_proj" not in name) and ("k_proj" not in name) and ("v_proj" not in name) and ("sigma" not in name):
                p.requires_grad = False
            elif name[:7] == 'encoder':
                p.requires_grad = False
                # We do not implement linear attention for encoders
            else:
                print(name)
