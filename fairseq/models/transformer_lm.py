...
from fairseq.random_feature_attention import RFADecoder, MLPDecoder
...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-rfa', action='store_true',
                            help='use rfa')
        parser.add_argument('--causal-proj-dim', type=int, metavar='D', default=256,
                            help='causal proj dim')
        parser.add_argument('--causal-tau', type=float, metavar='D', default=1.0,
                            help='causal tau')
        parser.add_argument('--reparam-proj', action='store_true',
                            help='reparam proj')
        parser.add_argument('--learned-tau', action='store_true',
                            help='learned tau')
        parser.add_argument('--init-scale', type=float, metavar='D', default=8.0,
                            help='init scale')
        parser.add_argument('--random-feature', type=str, metavar='D', default='rrf',
                            help='random feature')
        parser.add_argument('--cuda-causal-rfa', action='store_true',
                            help='use cuda rfa')
        parser.add_argument('--norm-rescale', action='store_true',
                            help='norm rescale')
        parser.add_argument('--attn-act', type=str, metavar='D', default='relu',
                            help='attention activation')
        parser.add_argument('--use-input-gate', action='store_true',
                            help='input gate')
        parser.add_argument('--attn-only-training', action='store_true',
                            help='only train attn parameters')
        # fmt: on
    ...
    def build_model(cls, args, task):
        ...
        if args.use_rfa:
            if args.random_feature == "mlp" or args.random_feature == "mlp-norm":
                decoder = MLPDecoder(
...
def base_lm_architecture(args):
    ...
    args.attn_only_training = getattr(args, 'attn_only_training', False)
